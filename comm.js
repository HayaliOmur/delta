var mirror = {
  ...
  connect: function (a) {
    ...
    this.socket = new ReconnectingWebSocket(
      "wss://cloud.achex.ca/JIMBOY3200" + this.token,
      null,
      {
...
function minimapCell(a, b, c, d) {
  ...
  this.setColor = function (e, f) {
    ...
    if (f.length == 7) this.customColor = f;
  };
  ...
  this.drawPosition = function (e, f, g, h, i) {
    ...
    if (this.nick.length > 0) {
      e.font =
        theme.miniMapNickFontWeight +
        " " +
        theme.miniMapNickSize +
        "px " +
        theme.miniMapNickFontFamily;
      e.textAlign = "center";
...
class Agartool {
  constructor(a) {
    ...
    this.hidden = false;
    this.profile =
      this.cn.tabName === "master"
        ? profiles.masterProfile
        : profiles.slaveProfile;
    this.publicIP = "";
    this.socket = {};
    this.finderSocket = {};
    this.playerID = null;
  ...
  onSpawn() {
    ...
    comm.cacheCustomSkin(
      profiles.masterProfile.nick,
      null,
      profiles.masterProfile.skinURL
    );
    comm.cacheCustomSkin(
      profiles.slaveProfile.nick,
      null,
      profiles.slaveProfile.skinURL
    );
  }
  destroy() {
    try {
  ...
  connect() {
    if (!this.ws) return;
    try {
      if (this.isSocketOpen()) this.closeConnection();
    } catch (a) {}
  ...
  mapConnect(a) {
    this.socket = io.connect(a.minimapServer, {
      ...
      query:
        "server=" +
        encodeURIComponent(a.agarServer) +
        "&tag=" +
        encodeURIComponent(this.clanTag),
    });
  ...
  readMessage(a) {
    ...
    switch (a.name) {
      case "add":
        if (!this.hidden) this.parseTeammate(a);

        break;
      case "remove":
        break;
      case "position":
        if (!this.hidden) this.parseTeammatePos(a);

        break;
      case "customSkins":
        if (!this.hidden) this.parseSkin(a);

        break;
      case "reset":
        break;
      case "chat":
        if (!this.hidden) this.readChatMessage(a);
  ...
  readMessage(a) {
    switch (a.getUint8(0)) {
      ...
      case 1:
        ...
        break;
      case 20:
        if (this.cn.tabName === "master") this.parseTeammate(a);

        break;
      case 30:
        if (this.cn.tabName === "master") this.parseTeammatePos(a);

        break;
      case 100:
        if (this.cn.tabName === "master") this.readChatMessage(a);
...
var comm = {
  ...
  updateTeamPlayer(a) {
    ...
    b !== null
      ...
      : ((c = new minimapCell(a.id, a.nick, a.skinName, a.skinUrl)),
  ...
  updateTeamPlayers() {
    ...
    for (const a of this.teamPlayers) {
      ...
      if (a.alive) {
        ...
        if (!this.isChatUserMuted(a.id)) this.addChatUser(a.id, a.nick);
      }
    }
  ...
  enterChatMessage() {
    if (!this.$messageBox.is(":visible")) {
      ...
      if (a.length) {
        this.tab.master.ogario.sendChatMessage(
          101,
          profiles.masterProfile.nick,
          a
        );
        this.tab.master.agartool.sendChatMessage(
          101,
          profiles.masterProfile.nick,
          a
        );
  ...
  muteChatUser(a) {
    ...
    toastr.error(
      dictonary.userMuted.replace(
        "%user%",
        "<strong>" + this.escapeHTML(b) + "</strong>"
      ) +
        ' <button data-user-id="' +
        a +
  ...
  unmuteChatUser(a) {
    ...
    if (b != -1) {
      ...
      toastr.info(
        dictonary.userUnmuted.replace(
          "%user%",
          "<strong>" + this.escapeHTML(this.chatMutedUsers[a]) + "</strong>"
        )
      );
  ...
  parseMessage(a) {
    ...
    if (settings.chatEmoticons) e = this.parseEmoticons(e);
  ...
  parseEmoticons(a) {
    return String(a)
      .replace(/\&lt\;3/g, "<3")
      .replace(
        /(O\:\)|3\:\)|8\=\)|\:\)|\;\)|\=\)|\:D|X\-D|\=D|\:\(|\;\(|\:P|\;P|\:\*|\$\)|\<3|\:o|\(\:\||\:\||\:\\|\:\@|\|\-\)|\^\_\^|\-\_\-|\$\_\$|\(poop\)|\(fuck\)|\(clap\)|\(ok\)|\(victory\)|\(y\)|\(n\)|\(angry\)|\(clown\)|\(crazy\)|\(devil\)|\(devil2\)|\(fb\)|\(google\)|\(ghost\)|\(heel\)|\(kiss\)|\(lipstick\)|\(rage\)|\(teacher\)|\(together\)|\(toothy\)|\(evil\)|\(baby\)|\(wow\))/g,
        function (b) {
          return (
            '<img src="https://cdnjs.cloudflare.com/ajax/libs/emojione/2.2.7/assets/svg/' +
            emojiChar[b] +
            '" alt="' +
            b +
            '" class="emoticon">'
  ...
  displayChatMessage(a, b, c, d, e) {
    ...
    if (c != 0 && c != this.playerID) {
      ...
      i =
        '<a href="#" data-user-id="' +
        c +
        '" class="mute-user fas fa-user-times"></a> ';
    }
    ...
    if (b == 101) {
      if (settings.showChatBox) {
        ...
        if (settings.chatSounds) this.playSound(this.messageSound);
      ...
      if (this.chatHistory.length > 15) this.chatHistory.shift();
    } else {
      if (b == 102) {
        if (settings.showChatBox) {
          $("#chat-box-messages").append(
          ...
          if (settings.chatSounds) this.playSound(this.commandSound);
  ...
  displayTop5() {
    ...
    for (let d = 0; d < c; d++) {
      ...
      a += `<div class="tl-player-mass">${this.shortMassFormat(
        this.top5[d].mass
      )}</div>`;
      settings.showTop5Sectors &&
        (a +=
          `<div class="tl-player-location">[` +
          this.calculateMapSector(this.top5[d].x, this.top5[d].y) +
          `]</div>`);
      a += `<div class="tl-player-nick">${this.escapeHTML(
        this.top5[d].nick
      )}</div>`;
      ...
      for (let b = 0; b < this.chatHistory.length; b++) {
        $("#messages").append(
          '<li><span class="message-nick">' +
            this.chatHistory[b].nick +
            ': </span><span class="message-text">' +
            this.chatHistory[b].message +
            "</span></li>"
        );
      }
      return;
  ...
  drawMiniMap() {
    ...
    this.currentSector = this.calculateMapSector(
      this.c.playerX,
      this.c.playerY,
      true
    );
    this.miniMapCtx.globalAlpha = 1;
    this.miniMapCtx.font =
      theme.miniMapFontWeight + " " + (b - 4) + "px " + theme.miniMapFontFamily;
    ...
    if (settings.showExtraMiniMapGuides) {
      var o = (this.c.playerX + h) * f,
        p = (this.c.playerY + j) * g,
        q =
          drawRender.canvasWidth /
          (this.c.mapMaxX - this.c.mapMinX) /
          drawRender.scale,
        r =
          drawRender.canvasHeight /
          (this.c.mapMaxY - this.c.mapMinY) /
          drawRender.scale,
    ...
    for (let F = 0, G = application.tabs.length; G > F; F++) {
      ...
      this.miniMapCtx.beginPath();
      this.miniMapCtx.arc(
        (application.tabs[F].playerX + h) * f,
        (application.tabs[F].playerY + j) * g,
        theme.miniMapMyCellSize,
        0,
        this.pi2,
        false
      );
    ...
    if (this.teamPlayers.length)
      for (let H = 0; H < this.teamPlayers.length; H++) {
        this.teamPlayers[H].drawPosition(
          this.miniMapCtx,
          this.c.mapOffset,
          f,
          this.privateMiniMap,
  ...
  drawSectors(a, b, c, d, e, f, g, h, i, j, k, l) {
    ...
    l
      ? (a.font =
          theme.sectorsFontWeight +
          " " +
          theme.sectorsFontSize +
          "px " +
          theme.sectorsFontFamily)
      : (a.font =
          theme.miniMapFontWeight +
          " " +
          ~~(0.4 * n) +
          "px " +
          theme.miniMapFontFamily);
    a.textAlign = "center";
  ...
  drawSelectedCell(a) {
    if (
      this.c.play &&
      this.c.playerSplitCells > 1 &&
      (settings.splitRange ||
        settings.oppColors ||
        settings.oppRings ||
        settings.showStatsSTE)
    ) {
      a.fillStyle = "#FFFFFF";
  ...
  setUi() {
    ...
    this.chatBoxInstance = OverlayScrollbars(
