var camera = new(class {
    ...
    calcPosition() {
        ...
        for (let h = 0; h < application.tabs.length; h++) {
            if (!application.tabs[h] || !application.tabs[h].play) {
                continue;
            }
        ...
        for (var e of application.tabs) {
            ...
            if (settings.cameraDelay === 0) {
                ...
                e.viewX = this.camX;
                e.viewY = this.camY;
            } else if (settings.cameraDelay !== 0) {
                var f = settings.cameraDelay;
                var g = settings.cameraDelay - 1;
...
var drawRender = {
    nickToColorTable: Object.create(null),
    canvas: null,
    ctx: null,
    canvasWidth: 0,
    canvasHeight: 0,
    ...
    setCanvas() {
        ...
        this.canvas.onmousemove = a => {
            for (let b = 0, c = application.tabs.length; c > b; b++) {
                this.mouseClientX = a.clientX * settings.renderQuality;
                this.mouseClientY = a.clientY * settings.renderQuality;
                if (b == 0) {
                    this.setCursorPosition(application.tabs[b]);
                }
            }
        };
    },
    ...
    setScale(a) {
        ...
        if (a.play) {
            this.scale = (c * this.scale + Math.min(64 / a.playerSize, 1) ** 0.2 * this.getZoom()) / b;
        } else {
            this.scale = (c * this.scale + a.scale * this.getZoom()) / b;
        }
    },
    calcScale() {
    ...
    renderFrame() {
        ...
        if (typeof spects !== 'undefined') {
            ...
            for (let spectator of spects) {
                spectator.clientDummy.time = Date.now();
                for (let cell of spectator.clientDummy.cells) {
                    cell.moveCell();
                }
            }

            // Draw full spectator cells
            const VIEWPORT_SAFETY_MARGIN = 18;
            const CALCULATED_VIEWPORT_EXTRA_SAFETY_MARGIN = 6;
            for (let spectator of spects) {
                if (!spectator.adjustedCoords) continue;
                //spectator.clientDummy.sortCells();
                cellLoop:
                    for (let cell of spectator.clientDummy.cells) {
                        if (!this.isInDisplay(cell.targetX, cell.targetY, cell.size)) continue;
                        if (spectator.circleInViewportOfEarlierSpectator(cell.targetX, cell.targetY, cell.size, VIEWPORT_SAFETY_MARGIN)) continue;
                        for (let nonGridSpectator of application.tabs) {
                            if (nonGridSpectator && nonGridSpectator.circleInViewport(cell.targetX, cell.targetY, Math.max(cell.size - VIEWPORT_SAFETY_MARGIN - CALCULATED_VIEWPORT_EXTRA_SAFETY_MARGIN, 0)))
                                continue cellLoop;
                        }
                        this.cellsFrame.push(cell);
        ...
        if (a.mapOffsetFixed === true && Settings.customMapLogoCanvas.complet === true) {
            ...
            var f = (a.mapMinY - a.mapMaxY) / 5 * 2.2;
        ...
        for (let B = 0, C = this.cellsFrame.length; C > B; B++) {
            if (this.isInDisplay(this.cellsFrame[B].targetX, this.cellsFrame[B].targetY, this.cellsFrame[B].size) === false) {
                continue;
            }
            this.renderedObjects++;
        ...
        if (settings.debug) {
            ...
            for (var b = 0; application.tabs.length > b; b++) {
                ...
                var l = d.protocol_viewY - d.viewportH2s;
                var m = d.protocol_viewX + d.viewportW2s;
                var n = d.protocol_viewY + d.viewportH2s;
    ...
    renderFrame___() {
        ...
        if (a.mapOffsetFixed && Settings.customMapLogoCanvas.complet) {
            ...
            var e = (a.mapMinY - a.mapMaxY) / 5 * 2.2;
        ...
        application.eachTabByPriority((p, q, r, s) => {
            for (const t of p.cells) {
                ...
                if (!this.isInDisplay(t.targetX, t.targetY, t.size)) {
                    continue;
                }
                this.renderedObjects++;
        ...
        if (settings.debug) {
            ...
            for (var b = 0; application.tabs.length > b; b++) {
                ...
                var j = a.protocol_viewY - a.viewportH2s;
                var k = a.protocol_viewX + a.viewportW2s;
                var l = a.protocol_viewY + a.viewportH2s;
    ...
    isInDisplay: function(a, b, c) {
        ...
        const e = (this.canvasHeight >> 1) / this.scale;
        if (a + c < this.camX - d || b + c < this.camY - e || a - c > this.camX + d || b - c > this.camY + e) {
            return false;
        }
        return true;
    },
    setAutoHideCellInfo(a) {
        if (a <= 40 || this.scale < 0.5 && a < 550 && a < 20 / this.scale) {
            return true;
        }
        return false;
    },
    ...
    drawSectors(a, b, c, d, e, f, g, h, i, j, k, l) {
        ...
        if (l || !l && settings.showMiniMapGrid) {
            ...
            this.drawMapBorders(a, b, e, f, g, h, i, k);
        }
        if (l) {
    ...
    drawFood(a, b) {
        if (!a || !tempsett.showFood || settings.autoHideFoodOnZoom && this.scale < 0.2) {
            return;
        }
    ...
    drawPieChart() {
        ...
        const b = Math.min(200, 0.3 * this.canvasWidth) / 200;
    ...
    drawBattleAreaOnMinimap(a, b, c, d, e, f) {
        ...
        if (this.battleAreaMap.width != b) {
            this.battleAreaMap.width = b;
            this.battleAreaMap.height = c;
        } else {
            this.battleAreaMapCtx.clearRect(0, 0, b, c);
        }
        let g = (Connection.battleRoyale.x + e) * d;
        let h = (Connection.battleRoyale.y + f) * d;
        let i = Connection.battleRoyale.radius * d;
    ...
    drawDangerArea(a, b, c, d, e, f, g, h, i, j) {
        if (Connection.battleRoyale.radius == Connection.battleRoyale.maxRadius || d <= 0) {
            return;
        }
        a.save();
        a.globalAlpha = j;
    ...
    drawSafeArea(a, b, c, d, e, f) {
        if (Connection.battleRoyale.state > 2 || d <= 0) {
            return;
        }
    ...
    drawGhostCells(a, b) {
        ...
        for (let d = 0; d < c.length; d++) {
            ...
            const e = c[d].x;
            const f = c[d].y;
            if (!this.isInDisplay(e, f, c[d].size)) {
                continue;
            }
            if (c.inView) {
                continue;
    ...
    init() {
        ...
        if (/firefox/i.test(navigator.userAgent)) {
            document.addEventListener('DOMMouseScroll', a.bind(this));
        } else {
            document.body.onmousewheel = a.bind(this);
        }
        this.setCanvas();
        this.resizeCanvas();
        this.preDrawPellet();
